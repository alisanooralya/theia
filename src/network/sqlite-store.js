...
const AUTH_DB_PATH = resolve(
  SETTINGS.dbPath?.replace('.db', '_auth.db') ?? './data/theia_auth.db'
);
...
export async function useSQLiteAuthState(sessionId = 'default') {
  ...
  const getCred = db.prepare(
    'SELECT value FROM auth_creds WHERE session_id = ? AND key = ?'
  );
  const setCred = db.prepare(
    'INSERT INTO auth_creds (session_id, key, value) VALUES (?, ?, ?) ON CONFLICT DO UPDATE SET value = excluded.value'
  );
  const getKey = db.prepare(
    'SELECT value FROM auth_keys WHERE session_id = ? AND type = ? AND id = ?'
  );
  const setKey = db.prepare(
    'INSERT INTO auth_keys (session_id, type, id, value) VALUES (?, ?, ?, ?) ON CONFLICT DO UPDATE SET value = excluded.value'
  );
  const delKeys = db.prepare(
    'DELETE FROM auth_keys WHERE session_id = ? AND type = ? AND id = ?'
  );
