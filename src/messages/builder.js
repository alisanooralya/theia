...
class AIRich extends BaseBuilder {
  ...
  addImage(imageUrl) {
    if (!(
      ...
      (Array.isArray(imageUrl) && imageUrl.every((v) => typeof v === 'string'))
    )) {
  ...
  addVideo(videoUrl) {
    if (!(
      ...
      (Array.isArray(videoUrl) && videoUrl.every((v) => typeof v === 'string'))
    )) {
  ...
  async send(
    jid,
    { forwarded, includesUnifiedResponse, includesSubmessages, ...options } = {}
  ) {
