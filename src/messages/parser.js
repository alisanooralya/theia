...
export async function parseMessage(raw, sock) {
  ...
  let phoneCandidate = [rawA, rawB].find((x) => x?.endsWith('@s.whatsapp.net')) ?? null
  const lidCandidate = [rawA, rawB].find((x) => x?.endsWith('@lid')) ?? null

  if (!phoneCandidate && lidCandidate && sock?.signalRepository?.lidMapping?.getPNForLID) {
    try {
      const resolvedPn = await sock.signalRepository.lidMapping.getPNForLID(lidCandidate)
      if (resolvedPn) phoneCandidate = jidNormalizedUser(resolvedPn)
    } catch {
      // biarkan fallback ke LID kalau resolusi gagal
    }
  }
  ...
  const mentions = await resolveMentions(content?.contextInfo?.mentionedJid, sock)

  return {
    key: raw.key, jid, jidAlt, sender, senderAlt, senderLid, fromMe, isGroup,
    type: innerType, text,
...
async function resolveMentions(jids, sock) {
  ...
  for (const j of jids ?? []) {
    ...
    if (norm.endsWith('@s.whatsapp.net')) { out.push(norm); continue }
    if (norm.endsWith('@lid') && sock?.signalRepository?.lidMapping?.getPNForLID) {
      try {
        const pn = await sock.signalRepository.lidMapping.getPNForLID(norm)
        if (pn) { out.push(jidNormalizedUser(pn)); continue }
      } catch {
        // biarkan fallback ke LID kalau resolusi gagal
      }
    }
