...
class InventoryModel {
  ...
  `);
  _removeQty = lazyPrepare(
    'UPDATE inventories SET quantity = quantity - @qty WHERE jid = @jid AND item_id = @itemId'
  );
  _deleteEmpty = lazyPrepare(
    'DELETE FROM inventories WHERE jid = ? AND quantity <= 0'
  );
  _countSlots = lazyPrepare(
    'SELECT COUNT(*) as count FROM inventories WHERE jid = ?'
  );
  _itemExists = lazyPrepare('SELECT id FROM items WHERE id = ?');

  getAll(jid) {
    return this._findAll().all(jid);
  }
  getItem(jid, itemId) {
    return this._findOne().get({ jid, itemId }) ?? null;
  }
  ...
  countSlots(jid) {
    return this._countSlots().get(jid)?.count ?? 0;
  }
}
