...
class GroupActivityModel {
  ...
  _findOne = lazyPrepare(
    'SELECT * FROM group_activity WHERE jid = ? AND user_jid = ?'
  );
  _top = lazyPrepare(
    'SELECT * FROM group_activity WHERE jid = ? ORDER BY xp DESC LIMIT ?'
  );
  _rank = lazyPrepare(`
  ...
  `);
  _allGroups = lazyPrepare(
    'SELECT jid, COUNT(*) as members FROM group_activity GROUP BY jid'
  );
