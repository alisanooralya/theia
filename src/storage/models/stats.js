...
class StatsModel {
  ...
  _recordBattle = lazyPrepare('UPDATE stats SET win = win + @win, loss = loss + @loss, win_streak = CASE WHEN @win = 1 THEN win_streak + 1 ELSE 0 END, updated_at = unixepoch() WHERE jid = @jid')
  _applyBuff = lazyPrepare('UPDATE stats SET buff_atk = buff_atk + @atk, buff_def = buff_def + @def, buff_exp_mult = @expMult, buff_expire = @expire, updated_at = unixepoch() WHERE jid = @jid')

  applyBuff(jid, { atk = 0, def = 0, expMult = 1, durationMs = 3_600_000 } = {}) {
    const expire = Math.floor(Date.now() / 1000) + Math.floor(durationMs / 1000)
