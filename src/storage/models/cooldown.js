...
class CooldownModel {
  ...
  _set = lazyPrepare(
    'INSERT INTO cooldowns (key, expires_at) VALUES (@key, @expiresAt) ON CONFLICT(key) DO UPDATE SET expires_at = excluded.expires_at'
  );
  _delete = lazyPrepare('DELETE FROM cooldowns WHERE key = ?');
  _cleanup = lazyPrepare(
    'DELETE FROM cooldowns WHERE expires_at < unixepoch()'
  );
