...
const DEFAULTS = {
  bot_name: null,
  bot_prefix: null,
  menu_url: 'https://github.com',
  menu_title: 'Theia',
  menu_desc: 'WhatsApp Bot Framework',
  thumbnail_url: '',
  bot_status: 'online',
  maintenance: '0',
  footer_text: '',
...
class BotConfigModel {
  ...
  _set = lazyPrepare(
    'INSERT INTO bot_settings (key, value, updated_at) VALUES (@key, @value, unixepoch()) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = unixepoch()'
  );
  ...
  set(key, value) {
    this._set().run({ key, value: String(value) });
  }
  reset(key) {
    this._del().run(key);
  }
  ...
  isMaintenanceMode() {
    return this.get('maintenance') === '1';
  }

  getMenuConfig() {
    return {
      url: this.get('menu_url') ?? 'https://github.com',
      title: this.get('menu_title') ?? 'Theia',
