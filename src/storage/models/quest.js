...
class QuestModel {
  ...
  findQuest(id) {
    return this._findQuest().get(id) ?? null;
  }
  allQuests() {
    return this._allQuests().all();
  }
  getProgress(jid, questId) {
    return this._findProgress().get({ jid, questId }) ?? null;
  }
    ...
    const resetAt =
      quest.type === 'story'
        ...
        : Math.floor(Date.now() / 1000) +
          (quest.type === 'weekly' ? WEEKLY_RESET : DAILY_RESET);
  ...
  claim(jid, questId) {
    return this._claim().run({ jid, questId }).changes > 0;
  }
  ...
  bulkUpsert(quests) {
    db.transaction(() => quests.forEach((q) => this.upsertQuest(q)))();
  }
}
