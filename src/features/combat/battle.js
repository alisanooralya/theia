...
class BattleService {
  fight(attackerJid, defenderJid) {
    ...
    const effAtk = (s) => s.buff_expire > now ? s.atk + (s.buff_atk || 0) : s.atk
    const effDef = (s) => s.buff_expire > now ? s.def + (s.buff_def || 0) : s.def
    ...
    db.transaction(() => {
      ...
      const expMult = winnerStats.buff_expire > now ? (winnerStats.buff_exp_mult || 1) : 1
