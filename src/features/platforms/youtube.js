...
class YoutubeService {
  ...
  _extract(playerData, videoId, opts) {
    ...
    if (audioOnly) {
      ...
      return {
        type: 'audio',
        url: best.url,
        filename: `yt_${_slug(title)}_${videoId}.${ext}`,
        title,
        author,
        duration,
    ...
    const audioAdaptive = adaptive
      .filter((f) => f.mimeType?.startsWith('audio/') && f.url)
      .sort((a, b) => (b.bitrate ?? 0) - (a.bitrate ?? 0))[0];
    if (!videoAdaptive?.url || !audioAdaptive?.url)
      throw new Error('Format adaptive video/audio tidak tersedia.');

    return {
      type: 'video',
      mode: 'adaptive',
      videoUrl: videoAdaptive.url,
      audioUrl: audioAdaptive.url,
      filename: `yt_${_slug(title)}_${videoId}.mp4`,
      title,
      author,
      duration,
      width: videoAdaptive.width,
      height: videoAdaptive.height,
      quality: `${videoAdaptive.height}p`,
      videoMimeType: videoAdaptive.mimeType,
      audioMimeType: audioAdaptive.mimeType,
      isShort,
    };
  }
  ...
  async toBuffer(url) {
    try {
      ...
      throw new Error(
        `Gagal download dari YouTube (${err.response?.status ?? 'timeout'}).`
      );
    }
  }
}
