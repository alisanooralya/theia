...
class InstagramService {
  ...
  _extractShareId(url) {
    return (
      url.match(
        /instagram\.com\/share\/(?:p\/|reel\/)?([A-Za-z0-9_-]+)/
      )?.[1] ?? null
    );
  }
  ...
  async _requestMobileApi(mediaId) {
    try {
      const { data } = await axios.get(
        ...
        { headers: mobileHeaders, timeout: 12_000 }
      );
  ...
  async _requestHTML(id) {
    try {
      const { data: html } = await axios.get(
        ...
        { headers: embedHeaders, timeout: 12_000 }
      );
  ...
  async _getGQLParams(id) {
    try {
      ...
      const lsd =
        getObjectFromEntries('LSD', html)?.token ||
        randomBytes(8).toString('base64url');
      const csrf = getObjectFromEntries(
        'InstagramSecurityConfig',
        html
      )?.csrf_token;
      const anon_cookie = [
        csrf && `csrftoken=${csrf}`,
      ...
      return {
        headers: {
          ...
          'X-FB-LSD': lsd,
          'X-CSRFToken': csrf,
          'X-Bloks-Version-Id': getObjectFromEntries(
        ...
        body: {
          ...
          lsd,
          jazoest:
            getNumberFromQuery('jazoest', html) ||
  ...
  _extractNewPost(data, id) {
    ...
    if (data.video_versions) {
      const best = data.video_versions.reduce((a, b) =>
        a.width * a.height < b.width * b.height ? b : a
      );
  ...
  async _getPost(id) {
    ...
    try {
      ...
      logger.warn({ err: err.message, id }, '[IG] getPost error');
    }
    if (!data)
      throw new Error(
