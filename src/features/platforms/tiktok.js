...
class TiktokService {
  async resolve(rawUrl, opts = {}) {
    ...
    if (!postId)
      throw new Error('Tidak bisa resolve postId dari URL tersebut.');
  ...
  async _resolvePostId(url) {
    ...
    if (shortMatch) {
      ...
      try {
        const res = await axios.get(`${base}${shortCode}`, {
          maxRedirects: 0,
          validateStatus: (s) => s >= 200 && s < 400,
          headers: { 'user-agent': UA.split(' Chrome/1')[0] },
          timeout: 10_000,
        });
  ...
  async _fetchDetail(postId) {
    ...
    try {
      ...
      if (videoDetail.statusMsg)
        throw new Error(`Post tidak tersedia: ${videoDetail.statusMsg}`);
      detail = videoDetail?.itemInfo?.itemStruct;
      if (!detail) throw new Error('itemStruct kosong');
    } catch (err) {
      throw new Error(
        'Gagal parse data video. Mungkin dihapus, private, atau restricted.'
      );
    }
    if (detail.isContentClassified)
      throw new Error('Konten ini dibatasi umur (18+).');
  ...
  _extract(detail, postId, opts = {}) {
    ...
    if (images && !audioOnly) {
      const imageUrls = images
        .map(
          (i) =>
            i.imageURL?.urlList?.find((p) => p.includes('.jpeg')) ??
            i.imageURL?.urlList?.[0]
        )
        .filter(Boolean);
    ...
    if (audioOnly) {
      ...
      if (fullAudio || !audioUrl) {
        audioUrl = detail.music?.playUrl;
      ...
      return {
        type: 'audio',
        url: audioUrl,
        filename: `${audioFilename}.${isMp3 ? 'mp3' : 'm4a'}`,
        title: detail.desc ?? '',
        author,
  ...
  async toBuffer(url, cookie = '') {
    try {
      ...
      throw new Error(
        `Gagal download video (${err.response?.status ?? 'timeout'}).`
      );
    }
  }
}
