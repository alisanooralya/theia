...
class FacebookService {
  async resolve(rawUrl, opts = {}) {
    ...
    let finalUrl;
    if (shortLink) {
      finalUrl = await this._resolveShortLink(shortLink);
      if (!finalUrl) throw new Error('Gagal resolve link fb.watch.');
    } else if (shareType)
      finalUrl = `https://www.facebook.com/share/${shareType}/${id}`;
  ...
  _parseUrl(url) {
    ...
    const shareMatch = url.match(
      /facebook\.com\/share\/([a-z])\/([A-Za-z0-9_-]+)/
    );
    if (shareMatch)
      return { shareType: shareMatch[1] || 'v', id: shareMatch[2] };
  ...
  async _resolveShortLink(shortLink) {
    try {
      const res = await axios.get(`https://fb.watch/${shortLink}`, {
        headers,
        maxRedirects: 0,
      ...
      if (res.headers?.link) {
        const m = res.headers.link.match(/<(.*?)\/?>/);
  ...
  async _fetchPage(url) {
    try {
      ...
      logger.error({ err: err.message, url }, '[FB] Fetch failed');
      return null;
    }
  }
