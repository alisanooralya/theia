...
function validateUrl(url, platform) {
  ...
  if (!raw)
    return {
      ok: false,
      error: `URL ${platform} tidak diberikan — minta user mengirim link-nya.`,
    };
  if (!/^https?:\/\//i.test(raw))
    return {
      ok: false,
      error: `URL ${platform} tidak valid (harus diawali http/https).`,
    };
  const key = platform.toLowerCase();
  if (!URL_PATTERNS[key]?.test(raw))
    return { ok: false, error: `URL bukan link ${platform} yang valid.` };
...
function guardSize(buf) {
  if (buf.length > MAX_MEDIA_BYTES)
    throw new Error(
      'File terlalu besar untuk dikirim ke WhatsApp (maks 60MB).'
    );
}
...
export const downloaderTools = [
  {
    ...
    description:
      'Download video/slideshow/audio dari link TikTok (tanpa watermark) dan kirim ke chat.',
    permission: 'user',
    parameters: {
      type: 'object',
      properties: {
        url: {
          type: 'string',
          description:
            'Link TikTok lengkap, contoh: https://vt.tiktok.com/xxx atau https://www.tiktok.com/@user/video/123',
        },
      },
      required: ['url'],
    ...
    async execute(args, ctx) {
      ...
      try {
        ...
        if (result.type === 'video') {
          ...
          return {
            success: false,
            error: 'Jenis media TikTok tidak dikenali.',
          };
        }
        return {
          success: true,
          message: 'Media TikTok berhasil dikirim.',
          data: {
            type: result.type,
        ...
        return {
          success: false,
          error: err.message || 'Gagal mengunduh TikTok.',
        };
      }
    },
  },
  {
    ...
    description:
      'Download media Instagram (post/reel/carousel) dan kirim ke chat.',
    permission: 'user',
    parameters: {
      type: 'object',
      properties: {
        url: {
          type: 'string',
          description:
            'Link Instagram lengkap, contoh: https://www.instagram.com/reel/xxx',
        },
      },
      required: ['url'],
      additionalProperties: false,
    ...
    async execute(args, ctx) {
      ...
      try {
        ...
        if (result.type === 'carousel') {
          ...
          return {
            success: false,
            error: 'Jenis media Instagram tidak dikenali.',
          };
        }
        return {
          success: true,
          message: 'Media Instagram berhasil dikirim.',
          data: { type: result.type },
        };
      } catch (err) {
        return {
          success: false,
          error: err.message || 'Gagal mengunduh Instagram.',
        };
      }
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        url: {
          type: 'string',
          description:
            'Link Facebook lengkap, contoh: https://www.facebook.com/xxx/videos/123 atau https://fb.watch/xxx',
        },
      },
      required: ['url'],
      additionalProperties: false,
    ...
    async execute(args, ctx) {
      ...
      try {
        ...
        return {
          success: true,
          message: 'Video Facebook berhasil dikirim.',
          data: { title: result.title || null },
        };
      } catch (err) {
        return {
          success: false,
          error: err.message || 'Gagal mengunduh Facebook.',
        };
      }
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        url: {
          type: 'string',
          description:
            'Link YouTube lengkap, contoh: https://www.youtube.com/watch?v=xxx atau https://youtu.be/xxx',
        },
      },
      required: ['url'],
      additionalProperties: false,
    ...
    async execute(args, ctx) {
      ...
      try {
        ...
        return {
          success: true,
          message: 'Video YouTube berhasil dikirim.',
          data: {
            title: result.title || null,
        ...
        return {
          success: false,
          error: err.message || 'Gagal mengunduh YouTube.',
        };
      }
    },
  },
