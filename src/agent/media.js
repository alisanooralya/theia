...
export function buildMultimodalContent(text, media) {
  ...
  if (media.kind === 'audio') {
    ...
    const fmt = media.mime.includes('mp3')
      ? 'mp3'
      : media.mime.includes('wav')
        ? 'wav'
        : 'mp3';
    const prompt =
      safeText || 'Transkrip audio ini dan jawab sesuai konteksnya.';
    return [
...
export async function downloadMedia(parsed, msg) {
  ...
  if (
    parsed.isMedia &&
    ['imageMessage', 'audioMessage', 'ptvMessage'].includes(parsed.type)
  ) {
    target = parsed.type;
    rawMsg = msg;
  } else if (
    parsed.quoted?.isMedia &&
    ['imageMessage', 'audioMessage', 'ptvMessage'].includes(parsed.quoted.type)
  ) {
  ...
  if (buffer.length > MAX_BYTES)
    throw new Error(
      'Media terlalu besar (maks 5MB). Coba kirim file yang lebih kecil.'
    );

  const fallback = MIME_BY_TYPE[target] ?? 'application/octet-stream';
  const mime =
    target === 'imageMessage' ? sniffMime(buffer, fallback) : fallback;
