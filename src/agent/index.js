...
export const agentService = {
  ...
  async handleMessage(parsed, msg, sock) {
    ...
    try {
      ...
      await sock
        .sendMessage(parsed.jid, { text: err.message }, { quoted: msg })
        .catch(() => {});
      return;
    }
    ...
    if (result.text) {
      await sock
        .sendMessage(parsed.jid, { text: result.text }, { quoted: msg })
        .catch((err) => {
          logger.warn({ err: err.message }, 'Agent reply send failed');
        });
    } else if (result.error) {
      await sock
        .sendMessage(parsed.jid, { text: result.error }, { quoted: msg })
        .catch(() => {});
    }
  },
};
