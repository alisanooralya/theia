...
export class Agent {
  ...
  async run(agentCtx, userContent) {
    ...
    try {
      let res = await geminiClient.chat({
        messages,
        ...
        if (batch.length === 0) {
          ...
          messages.push({
            ...
            content: JSON.stringify({
              success: false,
              error: 'Batas tool calls tercapai. Rangkum hasil yang sudah ada.',
            }),
          });
      ...
      logger.warn(
        { err: err.message, userId: agentCtx.userId },
        'Agent run failed'
      );
  ...
  async _executeToolCall(agentCtx, toolCall) {
    ...
    try {
      ...
      return {
        success: false,
        error: 'Argumen tool tidak valid (bukan JSON).',
      };
    }
    ...
    if (!canUseTool(agentCtx.level, tool)) {
      return {
        success: false,
        error: 'Akses ditolak: tool ini memerlukan permission lebih tinggi.',
      };
    }
  ...
  _normalizeResult(out) {
    ...
    if (json.length > TOOL_RESULT_MAX) {
      return {
        ...
        message: `${result.message ?? ''} [hasil dipotong]`.slice(
          0,
          TOOL_RESULT_MAX
        ),
      };
