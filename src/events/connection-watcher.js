...
export async function onConnectionUpdate(update, restart, sock) {
  ...
  if (connection === 'close') {
    ...
    logger.warn(
      { statusCode, reason: resolveReason(statusCode) },
      'Connection closed'
    );
    ...
    switch (statusCode) {
      ...
      case 401:
        if (!everConnected) {
          logger.warn(
            'Session rejected before first connect — resetting session and retrying'
          );
          try {
            await fsp.rm(SETTINGS.sessionPath, {
              recursive: true,
              force: true,
            });
          } catch {}
