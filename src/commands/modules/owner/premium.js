...
export default {
  ...
  async execute(ctx) {
    ...
    const targetJid =
      ctx.mentions[0] ?? (ctx.args[1] ? phoneToJid(ctx.args[1]) : ctx.sender);
    ...
    if (sub === 'add') {
      ...
      return ctx.reply(
        `✅ @${targetJid.split('@')[0]} dapat premium ${duration} hari.`,
        { mentions: [targetJid] }
      );
    }
    ...
    if (sub === 'del' || sub === 'remove') {
      ...
      return ctx.reply(`✅ Premium @${targetJid.split('@')[0]} dicabut.`, {
        mentions: [targetJid],
      });
    }

    ctx.fail('Usage: `!premium add <@tag> [hari]` | `!premium del <@tag>`');
  },
};
