...
import { HISTORY_DISPLAY } from '#features/economy/config/market-config.js';
...
function marketView(list, coin) {
  ...
  if (coin !== null) lines.push(`💰 Coin: *${money(coin)}*`);
...
export default {
  ...
  async execute(ctx) {
    ...
    const wallet = await rpgCoinModel.getWallet(ctx.sender);
    return ctx.reply(marketView(list, wallet ? wallet.coin : null));
  },
};
