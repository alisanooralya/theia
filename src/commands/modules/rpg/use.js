import { statsModel, inventoryModel, itemModel } from '#storage/models/index.js'
...
export default {
  ...
  async execute(ctx) {
    ...
    if (item.category !== 'consumable') ctx.fail('❌ Hanya item consumable yang bisa dipakai.')
    ...
    if (data.heal) {
      ...
      statsModel.applyBuff(ctx.sender, { atk: data.atk, durationMs: BUFF_DURATION })
      msg = `⚔️ ATK +${data.atk} selama 1 jam!`
    } else if (data.def) {
      statsModel.applyBuff(ctx.sender, { def: data.def, durationMs: BUFF_DURATION })
      msg = `🛡️ DEF +${data.def} selama 1 jam!`
    } else if (data.mult) {
      statsModel.applyBuff(ctx.sender, { expMult: data.mult, durationMs: BUFF_DURATION })
