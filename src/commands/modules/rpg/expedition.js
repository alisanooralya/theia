...
export default {
  ...
  cooldown: EXPEDITION_COOLDOWN_MS,
  manualCooldown: true,
    ...
    if (sub === 'claim') {
      ...
      return ctx.reply(
        [
          ...
          '',
          `⏱️ Expedition berikutnya: ${F.formatDuration(EXPEDITION_COOLDOWN_MS)} lagi.`,
        ].join('\n')
      );
    }
    ...
    if (!category)
      return ctx.fail(
        [
          ...
          '',
          ...Object.entries(EXPEDITIONS).map(
      ...
      return ctx.fail(
        [
          ...
          ...Object.entries(category.options).map(
            ([key, item]) =>
              `- \`.expedition ${sub} ${key}\` — ${optionLine(item)}`
          ),
          '',
          'Atau ketik `.expedition` untuk daftar lengkap.',
        ].join('\n')
