  ...
  async execute(ctx) {
    ...
    try {
      ...
      if (ctx.quoted?.isMedia && ctx.quoted.raw) {
        buffer = await ctx.downloadMediaMessage(ctx.quoted.raw, 'buffer', {}, { logger: undefined })
      }
      if (!buffer) buffer = await ctx.downloadMedia()
      if (!buffer) return ctx.reply('Gagal download media.')

      const meta = ctx.rawArgs?.trim() ? { packName: ctx.rawArgs.trim(), packPublish: ctx.pushName || 'Theia', emojis: ['🤖'] } : {}
