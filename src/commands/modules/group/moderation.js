...
function getTargets(ctx) {
  ...
  if (ctx.quoted?.sender && !ctx.quoted.sender.endsWith('@g.us'))
    targets.add(normalizeJid(ctx.quoted.sender));
  return targets.size ? [...targets] : null;
}

function usageReply(ctx, cmd, example) {
  ctx.fail(
    `Usage: \`${cmd} @tag\` atau reply pesan target lalu ketik \`${cmd}\`${example ? `\nContoh: \`${example}\`` : ''}`
  );
}
...
export const promoteCommand = {
  ...
  async execute(ctx) {
    ...
    try {
      ...
      const names = targets.map((j) => `@${j.split('@')[0]}`).join(', ');
      await ctx.reply(`✅ ${names} sekarang jadi admin!`, {
        mentions: targets,
      });
    } catch (err) {
