...
export default {
  ...
  async execute(ctx) {
    if (!geminiClient.isConfigured())
      return ctx.reply('AI belum dikonfigurasi. Hubungi owner.');
    const now = Date.now();
    const cdKey = ctx.jid;
    if (lastRun.has(cdKey) && now - lastRun.get(cdKey) < COOLDOWN_MS) {
      const sisa = Math.ceil(
        (COOLDOWN_MS - (now - lastRun.get(cdKey))) / 60000
      );
    ...
    if (msgs.length < 10)
      return ctx.reply(
        `Belum cukup pesan untuk diringkas. Perlu 10, baru ada ${msgs.length}.`
      );
