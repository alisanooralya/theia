...
function getHealth(jid, groupJid) {
  const row = db.prepare(`SELECT COALESCE(SUM(damage), 0) as d FROM warns WHERE jid = ? AND group_jid = ?`).get(jid, groupJid)
...
export default {
  ...
  async execute(ctx) {
    const targetJid = ctx.mentions[0]
      ...
      ?? (ctx.args[0] && ctx.args[0].includes('@') ? phoneToJid(ctx.args[0]) : null)
    if (!targetJid) ctx.fail(`Usage: \`.warn @tag <nomor>\` atau reply pesan target lalu \`.warn <nomor>\`\n\nPilih nomor alasan:\n${reasonsList()}`)
    ...
    if (health <= 0) {
      try { await ctx.sock.groupParticipantsUpdate(ctx.jid, [targetJid], 'remove') } catch (err) {
        logger.warn({ err }, 'Failed to kick warned user')
      }
      db.prepare(`DELETE FROM warns WHERE jid = ? AND group_jid = ?`).run(targetJid, ctx.jid)
      return ctx.reply(`@${targetJid.split('@')[0]} health 0 dan di-kick!`, { mentions: [targetJid] })
    }
