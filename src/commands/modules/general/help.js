...
async function isPrivileged(ctx) {
  ...
  if (ctx.isGroup) {
    try {
      ...
      const candidates = [ctx.sender, ctx.msg?.senderAlt, ctx.msg?.senderLid, ctx.raw?.participant]
...
export default {
  ...
  async execute(ctx) {
    ...
    for (const cat of categories) {
      ...
      const lines = cmds.map(cmd => `│ • \`${prefix}${cmd.name}\` — ${cmd.description ?? 'No description'}`)
      sections.push([`┌ • ${icon} ${cat.toUpperCase()}`, ...lines, `╰───────···`].join('\n'))
    }
